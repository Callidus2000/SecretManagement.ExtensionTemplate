...
$script:ModuleVersion = (Import-PowerShellDataFile -Path "$($script:ModuleRoot)\SecretManagement.þnameþ.psd1").ModuleVersion
...
if ($importIndividualFiles)
{
	...
	foreach ($function in (Get-ChildItem "$ModuleRoot\SecretManagement.þnameþ.Extension\functions.sharedinternal" -Filter "*.ps1" -Recurse -ErrorAction Ignore)) {
		. Import-ModuleFile -Path $function.FullName
	}
