...
$parentModule = 'SecretManagement.ExtensionTemplate'
if (-not $ModuleName) { $ModuleName = 'SecretManagement.ExtensionTemplate.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\SecretManagement.ExtensionTemplate'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\SecretManagement.ExtensionTemplate.psd1"
$prereqHash = @{
